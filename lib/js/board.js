import { getPostContent, getPostCode } from './post-data.js';
	...
	let posts = [
		...
		{
		    id: 14,
		    title: "풀스택 동적 스케줄러 개발기: Spring Boot, Quartz, React로 배우는 실전 프로젝트",
		    author: "Rabbit",
		    date: "2025-09-16",
		    image: "lib/img/spring_boot.png", // Placeholder image
		    content: getPostContent(14),
		    code: getPostCode(14)
		},
		{
		    id: 15,
		    title: "Spring Boot 애플리케이션 Dockerizing",
		    author: "Rabbit",
		    date: "2025-09-19",
		    image: "lib/img/docker_spring.png",
		    content: getPostContent(15),
		    code: getPostCode(15)
		},
		{
			id: 16,
			title: "배치 처리 최적화 & Delta Sync",
			author: "Rabbit",
			date: "2025-10-23",
			image: "lib/img/batch.png",
			content: getPostContent(16),
			code: getPostCode(16)
		},
		{
			id: 17,
			title: "낙관적 잠금( Optimistic Lock )",
			author: "Rabbit",
			date: "2025-10-27",
			image: "lib/img/OL.png",
			content: getPostContent(17),
			code: getPostCode(17)
		},
		{
			id: 18,
			title: "JPA N+1 문제 해결 ( Fetch Join 활용 )",
			author: "Rabbit",
			date: "2025-10-29",
			image: "lib/img/JPA N+1.png",
			content: getPostContent(18),
			code: getPostCode(18)
		}
	...
	function renderBoard(container) {
		...
		`;
		boardContainer.appendChild(searchContainer);
        ...
        // 검색 버튼 이벤트 리스너 추가
        const searchButton = boardContainer.querySelector('#search-button');
        searchButton.addEventListener('click', () => {
        	const searchTerm = boardContainer.querySelector('#search-input').value.toLowerCase();
        	const filteredPosts = posts.filter(post => post.title.toLowerCase().includes(searchTerm));
        	currentPage = 1; // 검색 시 첫 페이지로 이동
        	renderPostList(filteredPosts);
        });

        const searchInput = boardContainer.querySelector('#search-input');
		...
		// 검색 입력 필드에서 Enter 키를 눌렀을 때 검색 버튼 클릭 이벤트를 발생시킵니다.
	...
	function renderPostList(postsToRender = posts) {
		...
		renderPagination(paginationContainer, postsToRender);
	 ...
	 */
	function renderPagination(container, postsToRender = posts) {
