...
gameContainer.addEventListener('click', (event) => {
	...
	const randomAlphabet1 = randomAlphabet();
	randomAlphabet1.addEventListener('click', (e) => {
    ...
    if (javaWordsLower.includes(formedWord.toLowerCase())) {
        ...
        for (let i = 0; i < children.length; i++) {
            if (children[i].nodeType === 1) { // 요소 노드만
                console.log(children[i].textContent, formedWord);
                if (children[i].textContent.toLowerCase().includes(formedWord)) {
