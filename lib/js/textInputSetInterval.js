...
function inter()
{
	return setInterval(typi,150);
}

function imgInter()
{
	return setInterval(imgStart,1000);
}

function imgAddInter()
{
	return setInterval(imgAdd,3000);
}

function interClick()
{
	const inputText = document.getElementById("inputText");
	if (inputText.value.includes("i love you"))
	{
		setInterval(typi,150);
	}
	else
	{
		if(inputText.value.includes("사랑해"))
		{
			alert("영어로 말해줘요");
		}
		else
		{
			alert("어처구니 없는 말은 삼가해주시기 바랍니다.");
			inputText.focus();
		}
	}
}
