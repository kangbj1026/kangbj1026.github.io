function createCommonHead() {
	...
	let textTitle = "Bunny Portfolio";
	if (location.pathname.includes("projects") === true)
	{
		textTitle = "Bunny Projects";
	}
	else if (location.pathname.includes("board") === true)
	{
		textTitle = "Bunny Board";
	}
	else if (location.pathname.includes("game") === true)
	{
		textTitle = "Java Word Drop";
	}
	else if (location.pathname.includes("java_spring_study_guide") === true)
	{
		textTitle = "Study Guide";
	}
	...
	// 나이트 모드 &  화이트 모드 스타일 시트
	const nightModeStyleLink = document.createElement('link');
	nightModeStyleLink.setAttribute('rel', 'stylesheet');
	nightModeStyleLink.setAttribute('href', 'lib/css/night-mode.css');
	head.appendChild(nightModeStyleLink);

	// 학습 가이드 전용 스타일 시트
	if (location.pathname.includes("java_spring_study_guide") === true) {
		const studyGuideStyleLink = document.createElement('link');
		studyGuideStyleLink.setAttribute('rel', 'stylesheet');
		studyGuideStyleLink.setAttribute('href', 'lib/css/java_spring_study_guide.css');
		head.appendChild(studyGuideStyleLink);
	}
}
